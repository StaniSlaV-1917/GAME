  ...
  <div class="auth-page-wrapper" ref="wrapperEl">
    ...
    <div class="auth-card">
      <div class="card-header">
        ...
        <p class="subtitle">Присоединяйтесь к нашему игровому сообществу</p>
      </div>
      <div v-if="error" class="error-banner"><p>{{ error }}</p></div>
      <form @submit.prevent="submit" class="auth-form">
        <div class="input-group">
          <label for="fullname">Полное имя</label>
          <input id="fullname" v-model="form.fullname" placeholder="Иванов Иван" required type="text" @focus="handleFocus(true)" @blur="handleFocus(false)"/>
        </div>
        <div class="input-group">
          <label for="email">Email</label>
          <input id="email" v-model="form.email" type="email" placeholder="you@example.com" required @focus="handleFocus(true)" @blur="handleFocus(false)"/>
        </div>
        <div class="input-group">
          <label for="phone">Телефон</label>
          <input id="phone" v-model="form.phone" placeholder="79991234567" required type="tel" @focus="handleFocus(true)" @blur="handleFocus(false)"/>
        </div>
        <div class="input-group">
        ...
        <div class="input-group">
          ...
          <input id="password_confirmation" v-model="form.password_confirmation" type="password" placeholder="••••••••" required @focus="handleFocus(true)" @blur="handleFocus(false)"/>
        </div>
        <button type="submit" class="submit-button" :disabled="isLoading">{{ isLoading ? 'Создание аккаунта...' : 'Зарегистрироваться' }}</button>
      </form>
      <div class="card-footer"><p>Уже есть аккаунт? <RouterLink to="/login" class="link">Войти</RouterLink></p></div>
    </div>
  </div>
</template>

<style scoped>
#particles-js-register {
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
